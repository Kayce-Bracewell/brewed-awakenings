...
const convertDataForAccounting = (product) => {
    let allUpperCase = ""
    for (let unit of product) {
        let upperDescription = unit.description
        let upperPrice = unit.price
        let upperType = unit.type
        allUpperCase += `${upperDescription} - ${upperPrice} - ${upperType}\n`
    }
    return allUpperCase
}
...
console.log(modifiedData.toUpperCase())
